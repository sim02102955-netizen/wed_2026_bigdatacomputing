# 시계열 데이터 기반 봇 탐지 데이터 셋 구축 프로그래밍
import numpy as np
...
def visualize_by_numpy(dataset):
  # 1. 표준편차 샘플 추출(160,)
  std_vals = dataset[:,-2]
  label_vals = dataset[:,-1]
  # 2. 논리인덱싱을 이용하여 그룹 봇과 사람으로 분리
  bots_std = std_vals[label_vals == 1]
  human_std = std_vals[label_vals == 0]

  # 시각화
  plt.figure(figsize=(10,6))
  plt.hist(bots_std, bins = 20, alpha = 0.7, label='봇', color='red')
